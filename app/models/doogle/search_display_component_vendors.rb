class Doogle::SearchDisplayComponentVendors
  ...
  def self.name_like(txt)
    Doogle::Display.select('distinct(displays.display_component_vendor_name)').where(['displays.display_component_vendor_name like ?', '%' + txt + '%']).order(:display_component_vendor_name).all.map(&:display_component_vendor_name)
  end
end
