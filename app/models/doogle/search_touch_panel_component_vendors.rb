class Doogle::SearchTouchPanelComponentVendors
  ...
  def self.name_like(txt)
    Doogle::Display.select('distinct(displays.touch_panel_component_vendor_name)').where(['displays.touch_panel_component_vendor_name ilike ?', '%' + txt + '%']).order(:touch_panel_component_vendor_name).all.map(&:touch_panel_component_vendor_name)
  end
end
