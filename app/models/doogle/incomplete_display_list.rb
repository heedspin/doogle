...
class Doogle::IncompleteDisplayList
  ...
  def xls_initialize
    xls_field('Customer') { |d| d.original_opportunity.try(:sales_customer_name) }
  ...
  def xls_data
  	...
  	Doogle::DisplayPrice.not_deleted.active.created_after(start_date).without_displays(@displays.values).includes(:display).where('display_prices.notes is null or display_prices.notes = \'\'').each do |p|
